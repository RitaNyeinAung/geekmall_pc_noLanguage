                    ...
                    <div class="central l">
                        ...
                        <div class="price">
                            ...
                            <div class="couponSliceDiv" v-if="storeCouponList.length != 0">
                                ...
                                <div class="couponSlice">
                                    <div v-if="showCoupon0" @mouseenter="updateXY(showCoupon0.name + '满' + showCoupon0.condition + '元使用' + startTimeFor  + '至' + endTimeFor, $event)" @mouseleave="updataFullFlag">
                                        <span class="coupon_frame">
                                            <s></s>
                                            <b></b>
                                            <span class="couponText">满{{storeCouponSlice1}}减{{storeCouponSlice0}}</span>
                                        </span>
                                    </div>
                                    <div v-if="showCoupon1" @mouseenter="updateXY(showCoupon1.name + '满' + showCoupon1.condition + '元使用' + startTimeFor  + '至' + endTimeFor, $event)" @mouseleave="updataFullFlag">
                                        <span class="coupon_frame">
                                            <s></s>
                                            <b></b>
                                            <span class="couponText">满{{storeCouponSlice3}}减{{storeCouponSlice2}}</span>
                                        </span>
                                    </div>
                                    <div v-if="showCoupon2" @mouseenter="updateXY(showCoupon2.name + '满' + showCoupon2.condition + '元使用' + startTimeFor  + '至' + endTimeFor, $event)" @mouseleave="updataFullFlag">
                                        <span class="coupon_frame">
                                            <s></s>
                                            <b></b>
                                            <span class="couponText">满{{storeCouponSlice5}}减{{storeCouponSlice4}}</span>
                                        </span>
                                    </div>
                                    <span @click="drawer = true" type="primary" style="margin-left: 16px;" class="Cmore_btn">更多>></span>
                                    <el-drawer
                                        title="优惠券"
                                        :visible.sync="drawer"
                                        :with-header="false">
                                        <div class="coupon_type_d">
                                            <span class="type_d"></span>
                                            可领取的券
                                            <span class="type_d"></span>
                                        </div>

                                        <div class="coupon_item_data" v-for="(voucher, index) in storeCouponList" :key="index">
                                            <div class="coupon_data_d">
                                                <div class="price_item">
                                                    <span class="item_yuan">￥</span>
                                                    <span class="item_yuan1">{{voucher.money}}</span>
                                                    <span class="item_yuan2">东券</span>
                                                </div>
                                                <div class="available_more">
                                                    <span class="more_condition">{{'满' + voucher.condition + '元使用'}}</span>
                                                    <span class="more_avai">仅可购买心相印品</span>
                                                </div>
                                                <div class="btn_get">立即领取</div>
                                                <div class="expaired_date">{{voucher.use_start_time | formatDate}} 至 {{voucher.use_end_time | formatDate}}</div>
                                            </div>
                                        </div>

                                        <div class="coupon_type_d1">
                                            <span class="type_d"></span>
                                            已领取的券
                                            <span class="type_d"></span>
                                        </div>
                            ...
                            @mouseleave="hoverShow = false">
                                ...
                                <div>
                                    ...
                                    <div v-if="reduction.length > 1">
                                        <div v-if="hoverShow" style="margin-top: 13px">
                                            <div class="activity" v-if="reduction.length != 0">
                                                <span class="man jian">{{reduction.name}}</span>
                                                <span class="both"
                                                    >单笔订单满￥{{ reduction.expense }}，立减￥{{
                                            ...
                                            <div class="notice_text">
                                                <img class="notice_img" src="../../assets/img/index.png" />
                                                <span>"满减""多买优惠"仅可任选其一，可在购物车更改</span>
                                            </div>
                                        </div>
                                        <div v-else>
                                            <div class="activity act_box" v-if="reduction.length != 0">
                                                <span class="man jian">{{reduction.name}}</span>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="dizhi">
                            ...
                            <p class="send-to">
                                ...
                                <span v-if="goodsData.freight_type == 0">
                                ...
                                <span v-if="goodsData.freight_type == 2">
                                    <span class="yunfei">运费：卖家包邮</span>
                                </span>

                                <span v-if="goodsData.is_presell == 0" class="delivery_time">有现货</span>
                                <span v-if="goodsData.is_presell == 1" class="delivery_time">预售商品</span>
                            ...
                            <p class="delivery_pay">
                                发货时间
                                <span>付款后&nbsp;<span>{{goodsData.presell_date}}</span>&nbsp;天内发货</span>
                            </p>
                        </div>
                        <div class="leiji">
                            <p>
                        ...
                        <div class="shuliang">
                            ...
                            <span class="l" v-if="goodsData.is_presell == 0">件 库存{{ goodsData.stock }}件</span>
                        </div>
                        <div class="buy">
      ...
      getFreightCal(){
        ...
        this.HTTP(this.$httpConfig.freightCalcus,data,'post')
          ...
          this.callFreMess = res.data.message;
            ...
            this.callFreStatus = err.data.status;
            this.callFreMess = err.data.message;
            console.log(err);
        })
      },
    getExpressInfo(){
    ...
    goodLuck(storeId, index) {
      ...
      this.storeId = Number(storeId);
            ...
            this.HTTP(
                ...
                .then(res => {
                    if (res.data.status == 1) {
                        ...
                        var time = new Date(Number(res.data.data.data[0].use_start_time) * 1000);
                        ...
                        this.startTimeFor = Y + "-" + m + "-" + d + " " + H + ":" + mi + ":" + s;

                        var time1 = new Date(Number(res.data.data.data[0].use_end_time) * 1000);
        ...
        addCar(price, storeId) {
            ...
            if (this.goodsNumber > this.goodsData.stock && this.goodsData.is_presell == 0) {
                alert("库存不足");
                return;
            }
            var params = {
        ...
        //商品数量加
        plus() {
            if (parseInt(this.goodsNumber) >= parseInt(this.goodsData.stock) && this.goodsData.is_presell == 0) {
                return;
            }
            this.goodsNumber++;
        },
        ...
        setAmount() {
            ...
            if (num >= stock && this.goodsData.is_presell == 0) {
        ...
        .price {
            ...
            .couponSliceDiv {
                ...
                s {
                    ...
                    width: 2px;
                    overflow: hidden;
                    background-image: url(../../assets/img/quan-arr.gif);
                }
                b {
                    right: -2px;
                    position: absolute;
