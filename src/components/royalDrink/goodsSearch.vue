      ...
      <div class="choose"> 
        ...
        <div class="g-main-n l">
          ...
          <div class="q_class_list l" :class="{active:onoff2}">
            <ul style="width: 965px" class="l" ref="priceHeight">
          ...
          <i style="margin-top: 20px" @click="down" v-show="find" class="el-icon-arrow-down"></i>
          <i style="margin-top: 20px" @click="down" v-show="lose" class="el-icon-arrow-up"></i>
          <i @click="open(2)" v-if="show1" :class="{'el-icon-arrow-up':onoff1}" class="el-icon-arrow-down g-btn"></i>
        </div>

        <div class="g-main-n l">
          <span class="l leftbg">状 &nbsp;态：</span>
          <div class="q_class_list l" :class="{active:onoff1}">
            <ul class="l" ref="priceHeight">
              <li class="l" v-for="(item, index) in selectGoods.presell" :key="index" @click="getPresell(item, index)"
                  :class="selected.presellIndex === index ? 'active' : '' ">{{item}}
              </li>
            </ul>
          </div>
          <i @click="open(2)" v-if="show1" :class="{'el-icon-arrow-up':onoff1}" class="el-icon-arrow-down g-btn"></i>
...
<script>
  ...
  export default {
    ...
    getPresell(item, index, class_id) {
      this.params.is_presell = index;
      this.selected.presellIndex = index;
      this.params.class_id = class_id;
      this.params.values = undefined;
      this.params.page = 1;
      // this.currentPage = 1;
    ...
    getClassName(id,class_name) {
      ...
      getTopClass() {
          this.HTTP(this.$httpConfig.getTopClass, {}, "post").then(res => {
              ...
              this.class_type = this.temporary.slice(0, 6);
              // this.class_type = res.data.data;
          });
      },
      down() {
          this.lose = !this.lose;
          ...
          if (this.find == true && this.lose == false) {
              ...
              this.class_type = this.temporary;
          }
      },
      ...
      getGoodsList(query) {
        ...
        this.HTTP(this.$httpConfig.SearchIndex+'/page/'+page,{
          title:title,
          type:type
        }, 'post').then((res) => {
          this.list = res.data.data.data;
