    ...
    <div class="center" v-if="cartShow">
      <div id="cart-goods-list" ref="listHeight" >
        ...
        <div class="dianpu" v-for="(store, index) in cartGoodsList" :key="index">
          ...
          <div class="bottom">
            ...
            <div v-for="(item, i) in store.goods" :key="i">
              <div class="minus_proGift">
                <div class="pro_gift">
                  <span class="original_price">￥{{ selectedPromo | keep2Num }}</span>
                  <br />
                  <span v-if="selectedPromo" class="gift_discount">减：<span>-￥{{ giftCount | keep2Num }}</span></span>
                  <span v-else></span>
                </div>
              </div>
              <div
              ...
              >
                  ...
                  <div class="activity-list">
                        <div class="pro">
                            <div class="activity" v-if="item.promotion.gift.length != 0">
                              <div v-for="(id, index) in item.promotion.gift.goods" :key="index">
                            ...
                            <div v-else class="activity" style="font-size: 12px; margin-top: 6px;">赠品被领光了  ，下次请提前订购。</div>
                        </div>
                    </div>
                ...
                <p class="l">
                  ...
                  <!-- <el-dropdown trigger="click" v-if="item.promotion.gift.length != 0">
                    <span class="el-dropdown-link">
                      促销<img class="down_arrow" src="../../assets/img/up.png">
                    </span>
                    <el-dropdown-menu slot="dropdown">
                      <el-dropdown-item v-for="(pData, index) in item.promotion.gift.goods" :key="index">
                        <el-radio v-model="radio" :label="1">满{{ pData.full }}元，数量{{pData.give_count}}赠完即止</el-radio>
                      </el-dropdown-item>
                    </el-dropdown-menu>
                  </el-dropdown> -->

                  <span @click="showPromotion" v-if="item.promotion.gift.length != 0" class="el-dropdown-link">
                    促销<img class="down_arrow" src="../../assets/img/up.png">
                  </span>
                </p>
                <div v-if="item.promotion.gift.length != 0" class="popup-div" v-show="showPromoData">
                  <!-- <div v-for="(pData, index) in item.promotion.gift.goods" :key="index" class="pop-data">
                    <el-radio class="radio-text" v-model="radio" :label="1">满{{ pData.full }}元，数量{{pData.give_count}}赠完即止</el-radio>
                  </div> -->
                  <div v-for="(pData, index) in promoData" :key="index" class="pop-data">
                    <el-radio class="radio-text" v-model="radioR" :label="index">{{pData.pd}}</el-radio>
                  </div>
                  <div class="button_data">
                    <button class="confirm_button" @click="usePromotion">确定</button>
                    <button class="cancel_button" @click="showPromotion">取消</button>
                  </div>
                </div>
                ...
                <div class="l jishu">
                  ...
                  ></el-input-number>
                  <br/>
                  <div v-if="item.is_presell == 0" class="delivery_time">有现货</div>
                  <div v-if="item.is_presell == 1" class="delivery_time">预售商品</div>
                </div>
      ...
      <div class="jiesuan" :class="{ 'fixed-bottom': fixedBtn }" v-show="cartGoodsList.length != 0">
        ...
        <div class="promotion_data">
          <span class="ml5">促销：<span>-￥{{ giftCount| keep2Num }}</span></span>
          <span class="ml6">优惠券：<span>-￥0.00</span></span>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    showPromotion() {
      this.showPromoData = !this.showPromoData;
    },
    usePromotion() {
      if(this.selectedPromo > 99) {
        this.giftCount = 5;
      }
      if(this.selectedPromo > 169) {
        this.giftCount = 10;
      }
      this.showPromoData = false;
      this.allPriceNum();
    },
    //选中的总价和数量
    allPriceNum() {
      ...
      for (var j = 0; j < this.goods.length; j++) {
        ...
        this.selectedPrice +=
          (parseFloat(this.goods[j].price_member) * this.goods[j].goods_num) - this.giftCount;
      }
    },
    // 店铺全选
    allShop(index) {
